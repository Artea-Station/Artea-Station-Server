...
/datum/game_mode/dynamic/proc/send_intercept()
	. = "<b><i>Artea Department of Intelligence Threat Advisory, Spinward Sector, TCD [time2text(world.realtime, "DDD, MMM DD")], [GLOB.year_integer+540]:</i></b><hr>"
	switch(round(shown_threat))
		if(0 to 19)
			...
			if (show_core_territory)
				...
				. += "Your sector's advisory level is Blue Star. At this threat advisory, the risk of attacks on Artea assets within the sector is minor, but cannot be ruled out entirely. Remain vigilant."
			else
				. += "Advisory Level: <b>Green Star</b></center><BR>"
				. += "Your sector's advisory level is Green Star. Surveillance information shows no credible threats to Artea assets within the Spinward Sector at this time. As always, the Department advises maintaining vigilance against potential threats, regardless of a lack of known threats."
		if(20 to 39)
		...
		if(66 to 79)
			...
			. += "Your sector's advisory level is Red Star. The Department of Intelligence has decrypted Cybersun communications suggesting a high likelihood of attacks on Artea assets within the Spinward Sector. Stations in the region are advised to remain highly vigilant for signs of enemy activity and to be on high alert."
		if(80 to 99)
