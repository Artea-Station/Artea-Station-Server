...
/obj/machinery/computer/records/proc/create_character_preview_view(mob/user)
	...
	var/atom/movable/screen/character_preview_view/new_view = new(null, user.client.prefs, user.client, assigned_view)
	new_view.update_body()
...
/obj/machinery/computer/records/proc/update_preview(mob/user, assigned_view, datum/record/crew/target)
	...
	var/atom/movable/screen/character_preview_view/old_view = user.client?.screen_maps[assigned_view]?[1]
	if(!old_view)
		return
