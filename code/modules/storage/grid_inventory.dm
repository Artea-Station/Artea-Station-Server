...
// ~storage component
///from base of datum/storage/can_user_take(): (mob/user)
...
/datum/storage/concrete/ms13
	grid = TRUE

/datum/storage/concrete/ms13/grid //main for MS13 backpacks
	screen_max_columns = 8
	screen_max_rows = 5
	screen_start_y = 7

/datum/storage/concrete/ms13/d_bag //main for Doctors bags
	screen_max_columns = 6
	screen_max_rows = 4
	screen_start_y = 6
	screen_start_x = 7

/datum/storage/concrete/ms13/firstaid //main for First Aid kits
...
/datum/storage/Initialize(datum/storage/concrete/master)
	...
	if(!.)
		return
	RegisterSignal(parent, COMSIG_STORAGE_BLOCK_USER_TAKE)
...
/datum/storage/orient2hud()
	...
	if(!master.grid)
		rows = clamp(max_slots, 1, screen_max_rows)
...
/datum/storage/can_be_inserted(obj/item/storing, stop_messages, mob/user, worn_check = FALSE, params, storage_click = FALSE)
	...
	if(worn_check && !worn_check(parent, user))
		host.add_fingerprint(user)
		return FALSE
	if(LAZYLEN(real_location.contents) >= max_slots)
		if(!stop_messages)
...
/datum/storage/proc/generate_bound_underlay(grid_width = 32, grid_height = 32)
	var/mutable_appearance/bound_underlay = mutable_appearance(icon = 'icons/hud/storage.dmi')
...
/atom/movable/screen/close/Click(location, control, params)
	...
	if(LAZYACCESS(modifiers, SHIFT_CLICK))
		...
		if(!istype(storage_master))
			return
		storage_master.hide_contents(usr)
