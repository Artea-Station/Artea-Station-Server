/obj/machinery/power/supermatter_crystal/process_atmos()
	...
	if(!absorbed_gasmix || !absorbed_gasmix.total_moles || isspaceturf(local_turf)) //we're in space or there is no gas to process
		...
		if(local_turf.simulated && produces_gas && power) //There is no gas to process, but we are not in a space turf. Lets make them.
			...
			absorbed_gasmix.adjustGas(GAS_PLASMA, max((device_energy) / PLASMA_RELEASE_MODIFIER, 0), FALSE) //Don't update just yet
			absorbed_gasmix.adjustGas(GAS_OXYGEN, max(((device_energy + TCMB) - T0C) / OXYGEN_RELEASE_MODIFIER, 0), TRUE) //Now we update the mixture
...
/obj/machinery/power/supermatter_crystal/proc/deal_damage(datum/gas_mixture/removed)
	...
	damage = max(damage + (max(combined_gas - (POWER_PENALTY_THRESHOLD * delam_damage_multipler), 0)/80) * DAMAGE_INCREASE_MULTIPLIER, 0)
