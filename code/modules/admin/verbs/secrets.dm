	...
	switch(action)
		...
		if("maint_access_engiebrig")
			...
			for(var/obj/machinery/door/airlock/maintenance/M in GLOB.machines)
				M.check_access()
				if (ACCESS_ARTEA_COMMON in M.req_access)
					M.req_access = list()
					M.req_one_access = list(ACCESS_SECURITY_HIGHSEC,ACCESS_ENGINEERING_LOWSEC)
		...
		if("maint_access_brig")
			...
			for(var/obj/machinery/door/airlock/maintenance/M in GLOB.machines)
				M.check_access()
				if (ACCESS_ARTEA_COMMON in M.req_access)
					M.req_access = list(ACCESS_SECURITY_HIGHSEC)
