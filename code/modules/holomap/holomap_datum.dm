...
/datum/station_holomap/proc/generate_legend(list/overlays_to_use = list())
	...
	for(var/list/overlay_name as anything in overlays_to_use)
		...
		if(overlay_name in disabled_overlays)
			var/image/disabled_marker = image('icons/obj/machines/holomap/8x8.dmi', "legend_cross")
...
/datum/station_holomap/proc/initialize_holomap_bogus()
	base_map = image('icons/obj/machines/holomap/480x480.dmi', "stationmap")

	var/image/legend = image('icons/obj/machines/holomap/64x64.dmi', "notfound")
	legend.pixel_x = 192
	legend.pixel_y = 224
