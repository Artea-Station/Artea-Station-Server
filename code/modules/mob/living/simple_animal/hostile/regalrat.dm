...
/mob/living/simple_animal/hostile/regalrat/proc/pry_door(target)
	...
	if(prying_door.hasPower())
		...
		playsound(src, 'sound/machines/door/airlock_alien_prying.ogg', 100, vary = TRUE)
...
/mob/living/simple_animal/hostile/rat/revive(full_heal_flags = NONE, excess_healing = 0, force_grab_ghost = FALSE)
	var/cap = CONFIG_GET(number/ratcap)
