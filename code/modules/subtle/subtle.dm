...
/datum/emote/living/subtle/run_emote(mob/user, params, type_override = null)
	...
	playsound(user, 'sound/voice/talksounds/subtle_sound.ogg', 75)
...
/datum/emote/living/subtler/run_emote(mob/user, params, type_override = null)
	...
	if(is_banned_from(user, "emote"))
		to_chat(user, span_warning("You cannot send subtler emotes (banned)."))
		return FALSE
	else if(user.client && user.client.prefs.muted & MUTE_IC)
	...
	var/ghostless = get_hearers_in_view(SUBTLE_DEFAULT_DISTANCE, user) - GLOB.dead_mob_list
	var/turf/source_turf = get_turf(user)
	for(var/mob/reciever in ghostless)
		reciever.show_message(subtler_message, alt_msg = subtler_message)
		reciever.playsound_local(source_turf, 'sound/voice/talksounds/subtle_sound.ogg', 75)
